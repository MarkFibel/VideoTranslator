import asclass CSRFTokenManagerCopy:
    """
    Менеджер CSRF токенов для защиты от DoS атак.
    Отслеживает состояние токенов и блокирует повторные запросы.
    """
    ...
    def _ensure_cleanup_task(self):
        ...
        if not self._cleanup_task_started:
            try:
                ...
                logger.debug("Event loop not running, cleanup task will start later")g
...
class CSRFTokenManager:
    ...
    def __init__(self):
        ...
        # Время жизни токена (по умолчанию 0.5 часа)
        self._token_lifetime = timedelta(hours=0.5)
        
        # Запускаем фоновую задачу очистки устаревших токенов
        asyncio.create_task(self._cleanup_expired_tokens())
    ...
    def acquire_token(self, token: str) -> bool:
        """
        Захватывает токен для обработки.
        ...
        # Добавляем токен в активные
        self._active_tokens.add(token)
        self._token_timestamps[token] = datetime.now()
        
        logger.info(f"Token {token[:8]}... acquired for processing")
        return True
    ...
    def release_token(self, token: str) -> None:
        ...
        if token in self._active_tokens:
            ...
            logger.info(f"Token {token[:8]}... released from processing")
        else:
    ...
    async def _cleanup_expired_tokens(self) -> None:
        ...
        while True:
            try:
                ...
                # Ждем 10 минут до следующей очистки
